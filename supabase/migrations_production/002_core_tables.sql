-- TELODOX Production Migration 002: Core Tables (Fixed)
...
CREATE TABLE tenants (
    ...
    subscription_plan VARCHAR(100), -- FK to subscription_plans.name (added after subscription_plans table)
    billing_email TEXT,
...
CREATE TABLE deal_assignments (
    ...
    -- Ensure assignment is within same tenant (enforced by trigger, not CHECK constraint)
...
CREATE TABLE msa_documents (
    ...
    -- Ensure document belongs to correct tenant (enforced by trigger, not CHECK constraint)
);
...
-- Add foreign key constraint from tenants to subscription_plans (after subscription_plans exists)
ALTER TABLE tenants 
