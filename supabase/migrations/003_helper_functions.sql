-- TELODOX Production Migration 003: Helper Functions
...
-- Function to validate tenant consistency for deal_assignments
CREATE OR REPLACE FUNCTION check_deal_assignment_tenant()
RETURNS TRIGGER AS $$
DECLARE
    app_tenant_id UUID;
BEGIN
    SELECT tenant_id INTO app_tenant_id FROM applications WHERE id = NEW.application_id;
    IF app_tenant_id IS DISTINCT FROM NEW.tenant_id THEN
        RAISE EXCEPTION 'Application and assignment tenants must match';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Function to validate tenant consistency for msa_documents
...
BEGIN
    SELECT tenant_id INTO app_tenant_id FROM applications WHERE id = NEW.application_id;
    IF app_tenant_id IS DISTINCT FROM NEW.tenant_id THEN
        RAISE EXCEPTION 'Application and MSA document tenants must match';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
