...
-- Helper function to get current user's tenant_id (in public schema)
...
-- Helper function to check if user is super admin (in public schema)
...
-- Helper function to check if user is tenant owner (in public schema)
...
CREATE POLICY "Tenant owners can update their own tenant" ON tenants
    FOR UPDATE USING (id = get_user_tenant_id() AND is_tenant_owner());
...
CREATE POLICY "Tenant owners can view users in their tenant" ON users
    FOR SELECT USING (tenant_id = get_user_tenant_id() AND is_tenant_owner());
...
CREATE POLICY "Tenant owners can manage users in their tenant" ON users
    FOR ALL USING (tenant_id = get_user_tenant_id() AND is_tenant_owner());
...
CREATE POLICY "Users can view active form templates in their tenant" ON form_templates
    FOR SELECT USING (tenant_id = get_user_tenant_id() AND is_active = true);
...
CREATE POLICY "Tenant owners can update applications in their tenant" ON applications
    FOR UPDATE USING (tenant_id = get_user_tenant_id() AND is_tenant_owner());
