...
-- Weekly growth metrics (Rewritten to be performant and correct)
...
new_tenants AS (
    SELECT
        COUNT(*) as count
    FROM tenants
    WHERE created_at >= (SELECT this_week_start FROM week_dates)
),
last_week_new_tenants AS (
    SELECT
        COUNT(*) as count
    FROM tenants
    WHERE created_at >= (SELECT last_week_start FROM week_dates)
      AND created_at < (SELECT this_week_start FROM week_dates)
),
new_users AS (
    SELECT
        COUNT(*) as count
...
completed_apps AS (
    SELECT
        COUNT(*) as count
    FROM applications
    WHERE completed_at >= (SELECT this_week_start FROM week_dates)
)
SELECT 
    (SELECT this_week_start FROM week_dates) as week_start,
    (SELECT count FROM new_tenants) as new_tenants_this_week,
    (SELECT count FROM last_week_new_tenants) as new_tenants_last_week,
    (SELECT count FROM new_users) as new_users_this_week,
    (SELECT count FROM last_week_new_users) as new_users_last_week,
    (SELECT count FROM new_apps) as new_applications_this_week,
    (SELECT count FROM completed_apps) as completed_applications_this_week;
...
COMMENT ON VIEW weekly_growth_metrics IS 'Week-over-week growth comparisons';
